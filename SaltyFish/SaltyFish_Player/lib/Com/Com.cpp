...
void Com::promiscuousCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
    static int rssiMatchCount = 0;

    if (type == WIFI_PKT_MGMT || type == WIFI_PKT_DATA) {
        const wifi_promiscuous_pkt_t* ppkt = (wifi_promiscuous_pkt_t*)buf;
        const uint8_t* srcMac = ppkt->payload + 10;

        const uint8_t targetMac[6] = {0x88, 0x13, 0xBF, 0x61, 0x84, 0xEC};
        ...
        if (memcmp(srcMac, targetMac, 6) == 0) {
            ...
            if (rssi > -60 && rssi < 0) {
                rssiMatchCount++;
                Serial.print("Valid RSSI count: ");
                Serial.println(rssiMatchCount);

                if (rssiMatchCount >= 5 && !isBrainNearby) {
                    isBrainNearby = true;
                    Serial.println("Brain is nearby");
                    delay(2000); // delay once after confirming
                }
            } else {
                rssiMatchCount = 0;
                isBrainNearby = false;
